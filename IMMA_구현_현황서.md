...
| `routers/vlm.py` | `/vlm/analyze-upload`: buyer 인증 + multipart 이미지 → Replicate VLM API 호출 → V.B raw JSON → `drawings`에 INSERT(file_sha256 + buyer_id) → drawing_id 반환. 동기 `def` 정의로 FastAPI thread pool 위탁. Replicate sync requests + time.sleep 영역이 async event loop 를 freeze 시키지 않도록 `image.file.read()` 동기 영역 사용. 응답 schema 는 `{drawing_id, drawing_no, file_uri, file_sha256, original_filename, prediction_id, status, vlm_output}` — V.B raw JSON 본문은 `vlm_output` 키로 반환되며 (`vlm_result_jsonb` 키는 부재), `quote-request.html` 의 AI 분석 결과 카드가 `data.vlm_output` 영역을 직접 hydrate |
| `routers/quotes.py` | `POST /api/quote` supplier 인증 + match_candidates.supplier_response='accepted' 검증 후 quote_responses INSERT. line_items INSERT 직전 process_code 방어층 — CSV string 영역은 `split(',')[0].strip()` 으로 첫 코드 추출, list 영역은 첫 원소 추출, 추출된 코드가 `process_catalog` 에 부재 시 NULL fallback. quote_line_items.process_code 단일 FK 위반 차단. 첫 견적 도착 시 RFQ `open → quoted` 자동 전이 + buyer 에게 `quote_received` 알림 |
| `routers/orders.py` | `POST /api/orders` (quote_id → orders INSERT + 선택 quote `accepted` + 나머지 `rejected` + rfqs `ordered` 전이 + supplier 에게 `order_confirmed` 알림). `GET /api/orders/{order_id}` 응답에 `order_id, status, total_price, currency_code, promised_delivery_date, company_id, company_name, rfq_id, rfq_status, buyer_id, quote_id` 포함 — frontend order-management 의 *주문 및 계약 정보* hydrate (`rfq_id` 로 `/api/rfq/{rfq_id}` 재호출하여 부품/수량/단가 표시) 및 payment-success 동적 hydrate 정합. 상태 전이 `PUT /api/orders/{order_id}/status`, 검수 `PUT /api/orders/{order_id}/inspect`, jobs 생성/진행도, 배송, 납품 이미지 영역 일괄 |
...
- `POST /api/quote` — supplier만 호출 가능. `match_candidates.supplier_response='accepted'` 검증 후 견적 INSERT. 견적 schema 는 `{rfq_id, company_id, total_price, estimated_lead_days, proposed_delivery_date, validity_until, assumptions, line_items[]}`. `assumptions` 는 supplier 5 항목 회신 영역(납기/금액/인증/후처리/메모) 중 `{certification, post_treatment, memo}` 3 항목을 JSON 문자열로 구조화 보존 (단일 text 컬럼 유지). `line_items[].process_code` 는 CSV/array 입력 시 첫 코드만 추출 + `process_catalog` FK 존재 검증, 부재 시 NULL fallback 적용하여 quote_line_items 단일 코드 FK 정합 보장. 첫 견적 도착 시 RFQ `open → quoted` 자동 전이. buyer에게 `quote_received` 알림.
- `GET /api/orders/{order_id}` — buyer/supplier/admin 역할별 소유권 검증. 응답에 `order_id, status, total_price, currency_code, promised_delivery_date, actual_delivery_date, production_start_date, nda_signed_at, contract_signed_at, created_at, updated_at, company_id, company_name, rfq_id, rfq_status, buyer_id, quote_id` 포함. `rfq_id` 응답 동봉으로 frontend 가 `/api/rfq/{rfq_id}` 재호출하여 *주문 및 계약 정보* (품명/수량/단가/납기) hydrate 가능. supplier `#orders` 폴링·payment-success 동적 hydrate 도 동일 응답 사용.
...
#### `companies.onboarding_status` 자동 전환

`routers/deps.py` 의 `_check_onboarding(conn, company_id)` 헬퍼가 4 조건 (1) `companies.business_registration_no IS NOT NULL`, (2) `equipment` 행 ≥1, (3) `company_material_capabilities` 행 ≥1, (4) `company_sites.region IS NOT NULL AND is_primary=true` 일괄 검사. 4 조건 모두 충족 시 `companies.onboarding_status` 를 `verified` 로 UPDATE 한 뒤 `_refresh_mv` 로 `company_capability_summary` MV 갱신. 부분 충족 (장비 또는 재질 보유 + 일부 미충족) 시 `draft → submitted` 전환. supplier 가 `supplier-settings#onboarding` 4 카드 (장비/재질/공정/사업자) 영역을 채우는 도중 `POST /api/equipment` / `POST /api/material-capability` / `POST /api/process-capability` / `PUT /api/company/profile` 마지막 호출 시점에 자동 verified 전환 가능 — admin verify 단계 없이도 매칭 노출 진입.
...
| supplier 온보딩 4 카드 흐름 | `supplier-register.html` 은 1 카드(가입 정보 + 회사명) 로 축약, 2 카드(업체 정보) + BRN/주소 영역 제거. 가입 완료 후 `initSupplierRegister` 가 `/supplier-settings#onboarding` 으로 redirect. `supplier-settings.html` 의 `#onboarding` 영역에 4 카드 신설: (1) 보유 장비 — `/api/equipment-categories` + `/api/equipment-models?category=` 셀렉트 + `POST /api/equipment` + 자동 매핑 공정 lock, (2) 처리 가능 재질 — `/api/material-categories` chip 토글 + `POST /api/material-capability`, EQUIPMENT_TO_MATERIAL_HINT(22 카테고리 dict) 로 장비 등록 시 추정 재질 자동 체크. 재질 chip 선택 상태는 `pendingMaterialCodes` Set 을 *source of truth* 로 두고 DOM `.selected` 클래스는 시각 표시 영역으로만 사용 — 저장 시 `pendingMaterialCodes` ∪ DOM `.selected:not(.is-locked)` 합집합 영역 (refresh 도중 클래스 누락 방어), POST 후 `lockedMaterialCodes` 와 비교하여 미반영 항목은 *선택 상태 유지* + warning toast. 재질 영역에는 *delete 후 insert* 패턴 부재 — 백엔드 `register_material_capability` 가 기존 비활성 row 재활성화 우선 (UPDATE `is_active=true`), 신규만 INSERT, (3) 추가 공정 — `/api/processes` chip + 장비 자동 매핑 공정 lock 표시 + `POST /api/process-capability`, service_mode 셀렉트 영역은 폐기되었으며 frontend 는 `in_house` default 영역으로 일괄 송신 (외주/사내 분리 영역은 Phase 2 로 미룸), (4) 사업자 정보 — BRN + 시·도(17 종) + city + address + 대표자명 + 우편번호 `PUT /api/company/profile`. `applyOnboardingStatus`가 banner badge (draft/submitted/verified/rejected) + 진행도 (`장비 N / 재질 N / BRN 유무 / region 유무`) 갱신. 백엔드 `_check_onboarding` 4 조건(장비/재질/BRN/region) 모두 충족 시 `draft/submitted → verified` 자동 전환 + `_refresh_mv` 로 `company_capability_summary` MV 갱신, 부분 충족 시 `draft → submitted` 전환 | 명세서 line 224 의 supplier 가입 영역 = 최소 정보로 한정, 온보딩 정보는 별도 4 카드 영역에서 수집. 장비 등록 시 자동 매핑되는 공정·재질을 lock 상태로 노출하여 사용자가 중복 입력하지 않도록 한다. 재질 chip pending Set source of truth 영역으로 refresh ↔ 사용자 토글 race 차단, service_mode 드롭다운 제거로 시연 영역의 *외주/사내* 결정 부담 감소 |
...
| quote_line_items.process_code 단일 FK 정합 (CSV → 첫 코드 추출 + 백엔드 방어층) | `matching.py:671` 의 `string_agg(DISTINCT rpp.process_code, ', ')` 결과가 buyer 매칭 응답의 `processes` 필드에 CSV string 으로 내려가는 영역 ↔ `quote_line_items.process_code` 단일 코드 FK (`process_catalog` 참조) 영역 사이 격차. supplier 견적 작성 시 frontend (`quotePayloadFromWorkbench`) 가 1 차로 `match.processes.split(',')[0].trim()` 으로 첫 코드 추출, backend (`routers/quotes.py`) 가 2 차로 CSV/array 정규화 + `process_catalog` 존재 검증 + 부재 시 NULL fallback 적용. 2 중 정합으로 FK 위반 차단 |
| assumptions JSON 구조화 (supplier 5 항목 회신) | supplier `#reply` 영역 5 항목 (납기 / 금액 / 인증 / 후처리·조립 / 작업 메모) 중 `{certification, post_treatment, memo}` 3 항목을 단일 `quote_responses.assumptions` text 컬럼에 JSON 문자열로 보존. buyer `order-management` 영역의 `parseQuoteAssumptions` helper 가 safe parse 후 *품질 인증 / 후처리·조립 / 작업 메모* 별도 행으로 구조화 표시. JSON parse 실패 시 구버전 자유 텍스트 fallback (`memo` 단일 키로 회수). 납기 / 금액은 별도 컬럼 (`proposed_delivery_date` / `total_price`) 로 분리 보존 |
| datetime KST 변환 (formatDateTime helper) | `imma-ui-utils.js` 의 `formatDateTime(value)` 가 `Date(value).toLocaleString('ko-KR', {year, month, day, hour, minute})` 영역으로 UTC ISO 8601 timestamp 를 KST 로 일괄 변환. 기존 `String(value).slice(0, 10)` / `slice(0, 16)` 패턴이 UTC 영역 그대로 노출하던 결함을 시정 — `notifications.created_at` 등 timestamp 노출 영역에 적용. `formatDate(value)` 는 일자만, `formatDateTime(value)` 는 분 단위까지 표시 |
| client-dashboard 최근 진행 현황 open/quoted 포함 | `initClientDashboard` 의 `recentStatuses` Set 에 `open, quoted` 추가 (기존 `ordered, in_production, inspection, shipped, delivered, completed` 6 상태 영역) — buyer 로그아웃 후 재로그인 시 *견적 작성 대기 / 견적 도착* 상태 RFQ 가 최근 진행 현황 카드에 즉시 노출, `/order-management?rfq_id=` 로 상세 진입 가능. STATUS_LABEL dict 에 `open` (견적 작성 대기 · 노란) + `quoted` (견적 도착 · 초록) 2 항목 추가 | buyer 흐름의 진입점 영역이 *발주 확정 이후* 만 노출하던 격차 해소. 견적 대기 영역 RFQ 의 재진입 경로 보강 |
| AI 분석 결과 카드 (quote-request) | `quote-request.html` 의 `#ai-result-card` 영역에 VLM 응답 직후 `title_block_1` (Part_Name / Material / Drawing_No) + `view_1.measures` 항목 수 + `notes_1.lines` 후처리 요약 5 항목 hydrate. 각 행의 *수정* 버튼 클릭 시 인라인 input 전환, 저장 시 `dataset.userEdited='true'` + `dataset.original` 보존. RFQ 제출 시 `collectAiUserEdits` 가 사용자 수정 항목만 모아 `client_notes.ai_user_edits` 영역에 저장, `pipeline_runner.py` 의 `client_material` fallback 영역이 사용자 수정 재질을 우선 lookup. RFQ 확정 버튼은 `#submit-rfq-btn` 영역 한정으로 hook | VLM 추출 결과의 1 차 확인 + 정정 영역을 매칭 직전에 buyer 에게 부여. 추출 정확도가 낮은 항목 (재질·후처리) 을 buyer 가 직접 정정함으로써 매칭 fail-rate 감소 |
| supplier #reply 5 항목 회신 + #orders 폴링 | supplier `supplier-workbench.html` 의 `#reply` 카드에 5 input 영역: `#reply-due-date` (납기 일자) / `#reply-amount` (총 견적 금액) / `#reply-certification` (품질 인증) / `#reply-post-treatment` (후처리·조립) / `#reply-memo` (작업 메모). `quotePayloadFromWorkbench` 가 5 항목 수집 후 `POST /api/quote` 로 송신, `assumptions` 영역에 JSON 구조화 통합. buyer `order-management.html` 의 `refreshRfqState` 가 `/api/rfq/{rfq_id}/quotes` + `/api/notifications` 를 5 초 주기 폴링 + 견적 카드 hydrate (도착 시 폴링 자동 중단) + supplier_accepted 영역 *수락 받음* 초록 배지. supplier 측 `refreshOrdersSection` 이 `/api/notifications` 의 `order_confirmed` 알림 5 초 주기 폴링 + `#order-accept-row` hydrate + 발주 수락 체크박스 hook 으로 `PUT /api/orders/{id}/status` (`contracting → ordered`) + `POST /api/jobs` 자동 생성 | RFQ → 매칭 → 견적 → 발주 → 작업 흐름의 양면 (buyer / supplier) hydrate 영역 일괄 정합. 폴링 주기 5 초는 WebSocket 도입 이전 영역의 잠정 정합 |
| payment-success 동적 hydrate (`?order_id=`) | `payment-success.html` 의 자체 `hydratePaymentReceipt` IIFE 가 `URLSearchParams` 로 `order_id` query 추출 후 `/api/orders/{order_id}` 호출, `#ps-order-no` 영역에 `PO-{short_id}` (order_id 첫 8 자) + `#ps-amount` 영역에 `formatCurrency(total_price, currency_code)` + `.success-desc` 영역에 `company_name` 발주 건 안내. order_id query 부재 시 *발주 정보 부재* 빈 안내, hydrate 실패 시 *발주 조회 실패* 안내. 정적 PO/금액 데모 영역 폐기 | 결제 완료 화면이 정적 PO 번호 / 금액 표시 → 실 order 데이터 기반 hydrate 로 전환. order-management 의 *결제하기* 버튼이 `order_id` query 를 동봉하여 redirect |
| 수량 100 정적 더미 일괄 폐기 | (1) `supplier-dashboard.html` 의 *최근 매칭 RFQ* tbody — 정적 *100EA* row 폐기 → `loadSupplierMatchesFromAPI` 가 빈 영역에 placeholder *수신 매칭 대기 중* 메시지 표시. (2) `supplier-workbench.html` `#rfq` 영역 tbody — 정적 100EA 더미 row 폐기, `currentMatches.length === 0` 분기에서 *수신 매칭 대기 중 — 클라이언트가 매칭 실행하면 자동으로 표시됩니다* 안내. (3) `client-fulfillment.html` 영역 — 정적 *수령 수량 100* 영역 폐기 → *수령 수량 확인 대기* placeholder. 동적 hydrate 영역은 `rfq_parts.quantity` 또는 `rfqs.order_quantity` 실 값 사용 | 시연 영역에서 정적 100EA 더미가 buyer 가 입력한 실제 수량 (예: 200EA) 과 충돌하던 결함 시정. supplier 측 매칭 미수신 영역도 명시적 안내로 *empty state* 영역 분리 |
| setup_db.py --reset 옵션 | `pipeline/setup_db.py` 의 `__main__` 영역에서 `sys.argv` 영역의 `--reset` 플래그 감지 시 `_reset_schema()` 우선 실행 (DROP SCHEMA IF EXISTS imma CASCADE → CREATE SCHEMA imma) 후 `setup_all()` 진입. `DATABASE_URL` 환경변수 부재 시 `RuntimeError`. Railway 배포 영역에서 *Custom Start Command* 를 `python pipeline/setup_db.py --reset && python -m uvicorn main:app ...` 영역으로 일시 변경하여 시연 직전 깔끔 초기화 후 원래 Start Command 로 복원 | 시연 직전 mock 데이터 / 시연 fixture 영역 정합. `--reset` 부재 시 기존 `setup_all` 만 실행되어 idempotent 영역 유지. CASCADE 영역으로 FK 의존성 일괄 해소 |
