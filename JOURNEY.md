...
가용성 점수도 단순 평균이 아니라, 사내 공정 가능 장비의 `equipment_daily_schedule.available_hours` 합산으로 계산한다. 납기 데이터가 없으면 0.7, 전 공정 외주 RFQ 는 0.9, 장비 0 대 신고 업체는 0.3 으로 분기한다.
...
502 응답은 toast 후 사용자 선택을 제공하고, 504/timeout 은 fixture fallback UI 를 즉시 노출한다. 응답 schema 는 `vlm_output` 키 단일 source 이며, quote-request 화면은 `data.vlm_output || data.vlm_result_jsonb` 양면 fallback 으로 `hydrateAiResultCard` 를 호출한다. fallback 경로(sample_00015 직접 호출)에서는 VLM 응답 없이 정적 fallback 흐름으로 진행된다.
...
동시에, 비활성 데모 JS(client.js, supplier.js, app.js, app_unified.js, shared-state.js, site-actions.js, site-actions-demo.js)를 디렉토리에서 물리적으로 제거했다. 비활성 데모 코드의 전역 submit/click intercept 가 실 API 흐름을 가로채는 위험을 파일이 남아 있는 것만으로도 발생시키기 때문이다. 활성/비활성 자원의 물리적 분리로 컨텍스트 노이즈와 회귀 위험을 동시에 해소했다.
...
**학습**: 디자인 보존과 실 API hydrate 는 별도 신설이 아닌 기존 직접 주입으로 양립한다. 비활성 코드는 "안 쓰이니까 괜찮다"가 아니라, 남아 있는 것 자체가 위험이다. 그리고 frontend 작업의 본질은 "예쁘게 만드는 것"이 아니라 "데이터 흐름이 끊기지 않게 하는 것"이다.
...
수량은 실 API hydrate 또는 `'-'` fallback 으로 표시하고, 시각은 KST 변환 helper 를 일괄 적용한다. process_code 는 frontend 첫 코드 추출 + backend 방어층(CSV/array 재정규화 + process_catalog 존재 검증)으로 정합성을 보장한다. supplier 견적 회신은 납기, 금액, 인증, 후처리, 메모 5 항목 구조화 형식이며, buyer 가 견적 카드에서 의미 있는 비교를 수행한다. `assumptions` JSON 구조화로 단일 text 컬럼에 3 항목(certification, post_treatment, memo)을 보존하고, buyer 쪽 `parseQuoteAssumptions` 가 safe parse 로 추출한다.
...
demo 페이지가 남아 있는 화면(결제, 메시징, supplier 검색, admin KPI)에서는 `.admin-demo-notice` 배지("Demo UI - 일부 데이터는 시연용 샘플입니다")를 명시하여 시연 흐름이 끊기지 않으면서도 실 API 와 데모의 경계를 사용자에게 알린다. 정적 demo 시각을 일괄 숨기면 시연 단절이 발생하므로, demo 카드를 그대로 노출하되 배지로 경계를 명시하는 방식을 택했다.
...
매칭은 단부품 RFQ 를 기준으로 설계했다. 다부품 RFQ 의 rfq_part_id 별 분리 매칭과 supplier 응답 경로 확장은 다음 단계에서 다룬다.

CMPC(company_material_process_capabilities) 결합 테이블은 350 행 시드가 준비되어 있으며, MV 와 SQL 을 재설계하면 재질-공정 결합 질의("이 업체가 이 재질에 대해 이 공정을 수행할 수 있는가")를 hard filter 에 통합할 수 있다. 현재 MV 는 재질과 공정을 독립 집계하는 구조이므로, 결합 단계는 다음 설계로 분리했다.

GDT 는 hard filter 가 아닌 소프트 시그널(리스크 태깅, RFQ 질문 생성, 업체 랭킹 가중치)로 도입하는 방향이다. 알림은 DB 5 초 폴링으로 동작하며, WebSocket 전환은 운영 부하 단계에 맞춰 진행한다. supplier 주문 목록은 `/api/notifications` 기반으로 제공하며, 전용 endpoint 는 다음 단계에서 신설한다.

결제, 메시징, supplier 정밀 검색은 데모 페이지로 제공하며, PG 연동과 WebSocket 메시징은 운영 단계에서 구현한다. admin 관제는 pending verify/reject 등 핵심 API 를 실연결하고, KPI 산출과 나머지 read-only 연결은 다음 단계로 분리했다. 시연 범위를 핵심 흐름에 집중하고, 나머지는 발표 슬라이드에서 데모로 명시하는 것이 안정성 면에서 더 낫다는 판단이다.

매칭 신호의 `match_candidates.explanation_jsonb` 영구 기록, snapshot 과 스케줄 자동 동기화, 장비 등록 시 non_machining(열처리/용접/주조) capability 자동 생성은 운영 단계에서 순차 도입한다.
...
이 보류들은 MVP 시점의 의식적 우선순위 결정이다. 시연 가능한 핵심 흐름을 우선하고, 운영 단계 확장을 분리했다.
...
되돌아보면, 다회 독립 검증을 통해 보강된 것들이 있다. 프롬프트/스키마의 enum 정합, 업체-재질-장비 구조의 일관성, 인증과 소유권 검증의 강화, 비활성 데모 JS 제거, async/sync 선택의 재조정, supplier 온보딩 흐름의 정비 등. 이 개선들은 어느 하나도 "한 번에 잡히는" 종류가 아니었다. 반복 검증이 품질을 만든다는 것을 몸으로 체감했다.
