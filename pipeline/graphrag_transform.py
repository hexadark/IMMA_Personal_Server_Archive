...
17. part_name 추출 우선순위:
  (a) title_block 내 Part_Name / Part_Title / Description 항목을 최우선으로 채택한다.
  (b) title_block 이 없을 때에만 도면 본문 최상위 부품명 라벨을 채택한다.
  (c) view.name (예: "C部詳細", "A断面詳細", "正面図", "Front View", "Section A-A") 은
      부분 상세의 도면 뷰 명칭이므로 part_name 으로 채택 금지.
  (d) 어디에서도 part_name 이 식별되지 않으면 part_name 을 빈 문자열로 둔다.
      "(추정)" 부착 금지.
18. 원문 보존 정책:
  ...
  (d) post_treatment 만 규칙 9·9-1에 따라 한국어로 작성한다 (사용자 인지용).
